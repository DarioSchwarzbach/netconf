...
/**
 * See <a href="http://tools.ietf.org/html/rfc6241#section-6">rfc6241</a> for
 * details.
 */
public class SubtreeFilter {
	private static final Logger LOG = LoggerFactory.getLogger(SubtreeFilter.class);

	public static Document applyRpcSubtreeFilter(Document requestDocument, Document rpcReply)
		...
		if (XmlNetconfConstants.URN_IETF_PARAMS_XML_NS_NETCONF_BASE_1_0.equals(operationNameAndNamespace.getNamespace())
			...
			// process subtree filtering here, in case registered netconf
			// operations do
	...
	/**
	 * Filters notification content. If filter type isn't of type "subtree",
	 * returns unchanged notification content. If no match is found, absent is
	 * returned.
	 * 
	 * @param filter
	 *            filter
	...
	private static void removeEventTimeNode(Document document) {
		final Node eventTimeNode = document.getDocumentElement().getElementsByTagName(PeriodicNotification.EVENT_TIME)
				.item(0);
		document.getDocumentElement().removeChild(eventTimeNode);
	}

	private static boolean isSupported(XmlElement filter) {
		return "subtree".equals(filter.getAttribute("type")) || "subtree"
				.equals(filter.getAttribute("type", XmlNetconfConstants.URN_IETF_PARAMS_XML_NS_NETCONF_BASE_1_0));
	}
	...
	private static Document filteredNotification(XmlElement filter, Document originalNotification)
			throws DocumentedException {
	...
	private static MatchingResult addSubtree2(XmlElement filter, XmlElement src, XmlElement dstParent)
		...
		if (matches != MatchingResult.NO_MATCH && matches != MatchingResult.CONTENT_MISMATCH) {
			...
			if (filterHasChildren) { // this implies TAG_MATCH
				...
				for (XmlElement srcChild : src.getChildElements()) {
					for (XmlElement filterChild : filter.getChildElements()) {
						MatchingResult childMatch = addSubtree2(filterChild, srcChild,
								XmlElement.fromDomElement(copied));
				...
				if (numberOfTextMatchingChildren == filter.getChildElements().size()) {
					// force all children to be added (to depth). This is done
					// by copying parent node to depth.
	...
	/**
	 * Shallow compare src node to filter: tag name and namespace must match. If
	 * filter node has no children and has text content, it also must match.
	 */
	...
	private static boolean prefixedContentMatches(final XmlElement filter, final XmlElement src)
			throws DocumentedException {
		...
		try {
			...
			// if we can't find namespace of prefix - it's not a prefix, so it
			// doesn't match
			return false;
		}

		final String prefix = prefixToNamespaceOfFilter.getKey();
		// If this is not a prefixed content, we do not need to continue since
		...
		final String unprefixedFilterContent = filter.getTextContent()
				.substring(prefixToNamespaceOfFilter.getKey().length() + 1);
		final String unprefixedSrcContnet = src.getTextContent()
				.substring(prefixToNamespaceOfSrc.getKey().length() + 1);
